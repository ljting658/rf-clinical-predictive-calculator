...
# ==================== SCI论文级界面配置 ====================
st.set_page_config(
    page_title="Random Forest Predictive Calculator (SCI)",
    page_icon="📊",
    layout="centered",
...
def load_model_and_config():
    # 替换为你的模型路径
    model_path = r"D:\AApython\两组数据集\random_forest_model.pkl"
    scaler_path = r"D:\AApython\两组数据集\rf_scaler.pkl"
    threshold_path = r"D:\AApython\两组数据集\rf_optimal_threshold.txt"
    features_path = r"D:\AApython\两组数据集\rf_features.txt"
...
# ==================== 界面设计 ====================
# 标题（符合SCI论文命名规范）
st.title("Random Forest Predictive Calculator for Clinical Outcome")
st.markdown("### Based on SCI Paper: [可替换为你的论文标题]")
st.markdown("---")
...
if st.button("Calculate Prediction"):
    ...
    prob_interpretation = f"Probability of adverse outcome: {pred_prob:.4f}"
...
# ==================== SCI论文必备：方法说明 ====================
st.markdown("---")
st.subheader("Methodology (For SCI Review)")
st.markdown("""
<div class="info-text">
1. Model: Random Forest Classifier (class_weight='balanced', random_state=42)<br>
2. Hyperparameters: {补充论文中的最优参数，如n_estimators=100, max_depth=10}<br>
3. Standardization: Z-score normalization (StandardScaler from scikit-learn)<br>
4. Threshold: Optimal cutoff based on Youden index (J = Sensitivity + Specificity - 1)<br>
5. Features: 13 clinical variables (详见论文表1)<br>
6. Validation: 5-fold stratified cross-validation + bootstrap 95% confidence interval<br>
</div>
""", unsafe_allow_html=True)
